...
var fetchSummonerStats = (summonerName, region) => {
    return new Promise((resolve, reject) => {
        request({url: lolAPI.getSummonerId(summonerName, region), json:true}, (error, response, body) => {
            if(error) {
                ...
                if(response && response.statusCode > 400) {
                    ...
                    resolve(fetchSummonerStatsById(summonerId, region))
                }
            }
        })
    })
...
var fetchSummonerStatsById = (summonerId, region) => {
    return new Promise((resolve, reject) => {
        request(lolAPI.getRankedStats(summonerId, region), (error, response, body) => {
            if(error) {
