        ...
        if (!json.hasOwnProperty(prop)) {
            ...
            if (Array.isArray(obj[prop])) {
                if (!Array.isArray(json[prop])) {
                    ...
                    obj[prop] = unpackArray(json[prop], obj[prop]);
                }
            }
            // Check if this property is a non-null, non-function object
            else if (typeof obj[prop] === 'object') {
                if (typeof json[prop] !== 'object') {
                    // throw error if object is expected but one is not provided.
                }
                else {
                    obj[prop] = unpackObject(json[prop], obj[prop]);
                }
            }
            // Check if this property is a function (which is what Classes are) and that
            // it is not a primitive type
            else if (typeof obj[prop] === 'function' && 
                     obj[prop] !== String && obj[prop] !== Number && obj[prop] !== Boolean) {
                // json property will never be a function, so just ensure it an object
                if (typeof json[prop] !== 'object') {
                    // throw error if object is expected but one is not provided.
                }
                else {
                    obj[prop] = unpack(json[prop], obj[prop]);
                }
            }
...
function unpackObject(jsonObj, objObj) {
    var obj = {};
...
function unpackArray(jsonArr, objArr) {
    var arr = new Array();
    ...
    if (Array.isArray(arrayType)) { 
        jsonArr.forEach(val => {
            arr.push(unpackArray(val, arrayType));
        });
    }
    // non-function object
        ...
        jsonArr.forEach(val => {
            ...
            arr.push(unpackObject(val, arrayType));
        }); 
    }
    // function, assume it's a class
    else if (typeof arrayType === 'function' && 
             arrayType !== String && arrayType !== Number && arrayType !== Boolean) {
        jsonArr.forEach(val => {
            arr.push(unpack(val, arrayType));
        }); 
    }
    // otherwise assume it's a primitive type
    else {
